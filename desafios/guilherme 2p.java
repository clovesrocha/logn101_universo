...
public class DesafioProdutos {
    ...
    public static void main(String[] args) {
        ...
        produtos.add(new Produto("Celular Gamer", 6500.0, true));
        produtos.add(new Produto("Capa Anti Impacto", 80.0, true));
        produtos.add(new Produto("Cadeira Office", 500.0, false));
        produtos.add(new Produto("Xbox Usado", 1200.0, true));
        produtos.add(new Produto("Cabo HDMI", 35.0, false));

        System.out.println("=== Lista Original de Produtos ===");
        produtos.forEach(System.out::println);

        // B) FILTRO COM CLASSE ANÔNIMA (apenas os que estão em estoque)
        List<Produto> produtosEmEstoque =
                ServicoFiltro.filtrar(produtos, new FiltroProduto<Produto>() {
                    @Override
        ...
        // C) FILTRO COM LAMBDA (produtos com preço > 1000)
        List<Produto> produtosCaros =
                ServicoFiltro.filtrar(produtos, p -> p.getPreco() > 1000.0);

        System.out.println("\n=== [C] Produtos com preço > R$1000,00 (Lambda) ===");
        produtosCaros.forEach(System.out::println);

        // D) FILTRO COM LAMBDA (fora de estoque OU preço < 50)
        List<Produto> filtroBonus =
                ServicoFiltro.filtrar(produtos,
                        p -> (!p.isEmEstoque()) || (p.getPreco() < 50.0));

        System.out.println("\n=== [D] Bônus: fora de estoque OU preço < R$50,00 ===");
